...
<head>
    ...
    <title>Sans Speed - Gangguan Jaringan</title>
    <!-- Pustaka Standar -->
...
@include('billing.partials.web-bootstrap')
    ...
    <header class="app-header">
        ...
        <div class="header-title">Gangguan Jaringan</div>
        <button class="theme-toggle" id="themeToggleBtn" title="Ganti Mode"><i class="fas fa-moon"></i></button>
    </header>

    <!-- Filter Tabs -->
    <div class="tabs-container">
        <button class="tab-btn active" onclick="filterTask('pending', this)">Menunggu</button>
        <button class="tab-btn" onclick="filterTask('proses', this)">Diproses</button>
    ...
    <div class="content-area">
        <div id="loadingIndicator" class="loading-state">
            ...
            <p style="margin-top: 10px;">Memuat data tugas...</p>
        </div>

        <div id="emptyIndicator" class="empty-state">
            <i class="fas fa-check-circle"></i>
            <h3>Tidak ada tugas</h3>
            <p>Anda tidak memiliki tugas dalam kategori ini.</p>
        </div>
    ...
    <div id="taskModal">
        <div class="modal-bottom" id="modalBottomContent">
            <h3 class="modal-title" id="mJudul">Judul Tugas</h3>
            <div class="modal-pelanggan"><i class="fas fa-user"></i> <span id="mNamaPelanggan">Nama Pelanggan</span>
            </div>

            <div
                style="background: rgba(245, 158, 11, 0.1); padding: 12px; border-radius: 10px; margin-bottom: 15px; font-size: 13px; color: var(--text-main); border-left: 3px solid var(--warning);">
                <i class="fas fa-info-circle" style="color: var(--warning);"></i> <span id="mDeskripsi">Deskripsi
                    keluhan...</span>
            </div>
            ...
            <form id="formUpdateTask">
                ...
                <div class="form-group">
                    <label class="form-label">Ubah Status Tugas</label>
                ...
                <div class="form-group">
                    ...
                    <textarea id="inCatatan" class="form-control" rows="3"
                        placeholder="Tulis kerusakan/tindakan yang dilakukan..."></textarea>
                </div>
    ...
    <script type="module">
        ...
        async function loadTasks() {
            try {
                ...
                allTasks = tasks.filter(t => {
                    if (t.jenisTask !== 'troubleshoot') return false;
        ...
        function renderTasks() {
            ...
            // Sort by tglDibuat desc
            filteredTasks.sort((a, b) => new Date(b.createdAt || b.tglDibuat) - new Date(a.createdAt || a.tglDibuat));
                ...
                const htmlStr = `
                    <div class="task-card ${cardClass}" onclick="bukaModal('${t.id}')">
                        <div class="task-header">
                            <div class="task-title">${t.judul || 'Tanpa Judul'}</div>
                            <div class="task-status ${badgeClass}">${badgeText}</div>
                        </div>
                        <div class="task-meta">
                            <div><i class="fas fa-user-tag" style="width:16px;"></i> ${t.namaPelanggan || 'Tanpa Nama'} (${t.idPelanggan || '-'})</div>
                            ...
                            <div><i class="far fa-clock" style="width:16px;"></i> Dibuat: ${tglText}</div>
                        </div>
                    </div>
                `;
        ...
        window.bukaModal = function (id) {
            ...
            document.getElementById('mJudul').innerText = t.judul;
            document.getElementById('mNamaPelanggan').innerText = t.namaPelanggan || '-';
            document.getElementById('mDeskripsi').innerText = t.deskripsi || '-';
        ...
        document.getElementById('formUpdateTask').addEventListener('submit', async (e) => {
            ...
            try {
                ...
                alert("Gagal update tugas: " + err.message);
            } finally {
                btn.innerHTML = '<i class="fas fa-save"></i> Simpan Laporan';
                btn.disabled = false;
            }
        });
